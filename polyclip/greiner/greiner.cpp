...
int GreinerHormann::boolop (BoolOpType op, Polygon& result)
{
	if ((subject.ncontours () > 0 || clipping.ncontours () > 0) && op != INTERSECTION)
...
int GreinerHormann::boolop (BoolOpType op, GreinerContour& gc1, GreinerContour& gc2, Polygon& result)
{
	// step 1
	if (!gc1.intersectBoundingbox (gc2))
		return 0;

	GetTicker().record("Before Finding");
	...
	do {
		if (!v1p->intersect) {
			...
			do {
				if (!v2p->intersect) {
					Point inter;
					int interCount = findIntersection(v1p->s, v2p->s, inter, inter);
					if (interCount ==1) {
						if (inter == v1p->s.p2 || inter == v2p->s.p2)
						{
							//continue;
						}
						else if (inter == v1p->s.p1)
						{
							nint++;
							v1p->intersect = true;
						...
						else if (inter == v2p->s.p1)
						{
							nint++;
							v2p->intersect = true;
							double distTov1p = (v1p->x - inter.x) * (v1p->x - inter.x) + (v1p->y - inter.y) * (v1p->y - inter.y);
							Vertex *x, *y;
							for (y = v1p->next; y->alpha < distTov1p; y = y->next);
							Segment noused;
							Vertex *v2_address = v2p;
							Vertex *v1_address = gc1.insert(Vertex(inter.x, inter.y, noused, true, distTov1p), y);
							v1_address->neighbor = v2_address;
							v2_address->neighbor = v1_address;
						}
						else {
	...
	GetTicker().record("Brute Force Finding");
